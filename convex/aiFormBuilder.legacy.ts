...
export const createChatWithStream = mutation({
	...
	returns: v.object({
		assistantMessageId: v.id('chatMessage'),
		streamId: v.string()
	}),
	handler: async (ctx, args) => {
		const user = await resolveIdentity(ctx)

		console.log('[AI Form Builder] createChatWithStream called', {
			formId: args.formId,
		...
		console.log('[AI Form Builder] Form lookup result', {
			formExists: !!form,
			formBusinessId: form?.businessId,
			requestedBusinessId: args.businessId,
			match: form?.businessId === args.businessId
		})

		if (!form) {
			console.error('[AI Form Builder] Form not found', { formId: args.formId })
			throw new Error('Form not found')
		}

		// Verify user has access to the business/organization
		if (user.subject !== args.businessId) {
			console.log('[AI Form Builder] User is not businessId owner, checking membership', {
				userId: user.subject,
				businessId: args.businessId
			})
			try {
				...
				console.log('[AI Form Builder] Membership verified successfully')
			} catch (error) {
				console.error('[AI Form Builder] Membership verification failed', {
					error: error instanceof Error ? error.message : error
				})
				throw error
			}
		} else {
			console.log('[AI Form Builder] User is businessId owner (personal account)')
		}

		// Verify form belongs to the business
		if (form.businessId !== args.businessId) {
			console.error('[AI Form Builder] Form businessId mismatch', {
				formBusinessId: form.businessId,
				requestedBusinessId: args.businessId
			})
		...
		console.log('[AI Form Builder] All checks passed, creating chat with stream', {
			formId: args.formId,
...
export const streamAIResponse = httpAction(async (ctx, request) => {
	...
	const generateChat = async (
		...
		try {
			console.log('[AI Form Builder] Starting generateChat', {
				chatMessageId: chatMessage._id,
				formId: chatMessage.formId,
				businessId: chatMessage.businessId,
				userId: chatMessage.userId
			})

			// Get the form first to determine businessId (for backward compatibility with old messages)
			const formDirect = await ctx.runQuery(internal.form.getById, {
				formId: chatMessage.formId
			})

			console.log('[AI Form Builder] Form direct query result', {
				formExists: !!formDirect,
				formBusinessId: formDirect?.businessId
			})

			if (!formDirect) {
				console.error('[AI Form Builder] Form not found in generateChat', {
					formId: chatMessage.formId,
					providedBusinessId: chatMessage.businessId,
					fallbackUserId: chatMessage.userId
				})
				await chunkAppender('Error: Form not found')
				return
			}

			// Use the form's actual businessId for subsequent queries
			const businessId = formDirect.businessId

			console.log('[AI Form Builder] Using businessId from form', {
				businessId,
				chatMessageBusinessId: chatMessage.businessId
			})

			// Get form and fields with correct businessId
			const form = await ctx.runQuery(internal.form.getInternal, {
				formId: chatMessage.formId,
				businessId: businessId
			})

			console.log('[AI Form Builder] Form query result in generateChat', {
				formExists: !!form,
				formId: chatMessage.formId,
				businessId: businessId
			})
			...
			if (!form) {
				console.error('[AI Form Builder] Form not found in getInternal', {
					formId: chatMessage.formId,
					businessId: businessId
				})
				await chunkAppender('Error: Form not found')
...
⚠️ CRITICAL FOR MULTIPLE FIELDS ⚠️
When a user requests multiple fields in ONE message (e.g., "add Name and Email fields"), you MUST call the createField tool MULTIPLE TIMES in the SAME response - once for each field. Do NOT combine fields into one call. Do NOT create only one field when multiple are requested.
...
- When user asks for MULTIPLE fields, call createField MULTIPLE times (once per field)
- Parse lists carefully: "Name" and "Email" are TWO separate fields, not one
...
User: "add fields: Name and Email"
Tool Call 1: createField with {title: "Name", type: "singleline", required: true}
Tool Call 2: createField with {title: "Email", type: "singleline", required: true}

User: "add fields: Name and Are you going to party? (Yes or No)"
Tool Call 1: createField with {title: "Name", type: "singleline", required: true}
Tool Call 2: createField with {title: "Are you going to party?", type: "select", required: false, options: ["Yes", "No"]}

User: "Add Name, Email, and Phone fields"
Tool Call 1: createField with {title: "Name", type: "singleline", required: true}
Tool Call 2: createField with {title: "Email", type: "singleline", required: true}
Tool Call 3: createField with {title: "Phone", type: "singleline", required: false}

User: "Are you going to party? (Yes or No)"
Tool Call: createField with {title: "Are you going to party?", type: "select", required: false, options: ["Yes", "No"]}
...
- "select" for: choosing from options like Yes/No, dropdown choices (use options array)
- "checkbox" for: single yes/no agreement checkboxes ("I agree to terms")
...
REMEMBER: ALWAYS include the actual values in your tool calls. NEVER call a tool with empty {} parameters.`
			...
			createField: {
				description: 'Create a new field in the form. IMPORTANT: Call this tool ONCE for EACH field you need to create. If the user asks for multiple fields, you MUST call this tool multiple times - once per field.',
				parameters: z.object({
